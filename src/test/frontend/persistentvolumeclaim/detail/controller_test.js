...
describe('Persistent Volume Claim Detail controller', () => {
  ...
  it('should initialize persistent volume claim controller', angular.mock.inject(($controller) => {
    let data = {};
    /** @type {!PersistentVolumeClaimDetailController} */
    let ctrl =
        $controller(PersistentVolumeClaimDetailController, {persistentVolumeClaimDetail: data});

    expect(ctrl.persistentVolumeClaimDetail).toBe(data);
  }));
});
